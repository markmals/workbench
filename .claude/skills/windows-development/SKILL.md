...
| Reactivity / MVVM   | MVVM Toolkit (`CommunityToolkit.Mvvm`)                         | [learn.microsoft.com/dotnet/communitytoolkit/mvvm](https://learn.microsoft.com/en-us/dotnet/communitytoolkit/mvvm/)                       |
...
The Windows app reaches the backend through a **typed client generated by Kiota** from the project's OpenAPI document, with **`HttpClient`** as the transport. You never assemble requests by hand or model the wire protocol yourself. **EF Core (SQLite)** is the on-device store — a local-first cache and offline source, not a second backend. Identity flows through **Clerk**; the generated client attaches the Clerk token. See `apps/windows/CLAUDE.md` for the wrapper that exposes idiomatic call sites on top of the generated client.
