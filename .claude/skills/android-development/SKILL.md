...
The Android app reaches the backend through a **typed client generated by the OpenAPI Generator** (Kotlin) from the project's OpenAPI document, with **Ktor** as the transport. You never assemble requests by hand. **Room** is the on-device store — a local-first cache and offline source, not a second backend. Identity flows through **Clerk**; the generated client attaches the Clerk token. See `apps/android/CLAUDE.md` for the wrapper that exposes idiomatic suspend call sites on top of the generated client.
