...
For every platform **not** selected, remove its artifacts using the map below. Watch the **shared-tooling cautions** — some permissions and the `chrome-devtools` CLI tooling are shared, so only remove them when _all_ sharing platforms are gone.
...
| --------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Web app         | `web-development`, `web-verification` ⚠️shared-website | `pnpm`,`node`,`vite`,`vitest` ⚠️shared-TS; `chrome-devtools` ⚠️shared-website | `stop-lint` `web` line                                              | Web rows/cols; the `apps/web` layout line; Web Apps STACK section            |
...
- **Browser verification** uses the `chrome-devtools` CLI — pinned in the root `mise.toml` as `npm:chrome-devtools-mcp` and driven by the `web-verification` skill, shared by **web app and website**. There's no committed MCP file to prune; keep the root-`mise.toml` pin, the `web-verification` skill, and the `Bash(chrome-devtools:*)` permission while _either_ survives, and drop all three only when both are gone.
