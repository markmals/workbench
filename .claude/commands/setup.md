...
- **Browser verification** uses the global `chrome-devtools` CLI (`mise use -g npm:chrome-devtools-mcp@latest`), driven by the `web-verification` skill — shared by **web app and website**. There's no committed MCP file to prune; keep the `web-verification` skill and its `Bash(chrome-devtools:*)` permission while _either_ survives, and delete both only when both are gone.
