...
| --------------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Web app         | `web-development`, `web-verification`                  | `pnpm`,`node`,`vite`,`vitest` ⚠️shared-TS                                                     | `format-on-edit` TS branch ⚠️shared; `stop-lint` `web` line                   | Web rows/cols; the `apps/web` layout line; Web Apps STACK section            |
...
| Apple           | `ios-development`, `ios-simulator-control`             | `xcrun simctl …`, `xcodebuild`, `swift-format`, `idb`                                         | `tuist-regen`; `format-on-edit` `.swift`; `stop-lint` `ios`; `block-generated` Tuist/xcodeproj | Apple row; iOS column; Apple STACK section; Xcode MCP bullet      |
...
| Linux           | `linux-development`                                    | `cargo`,`rustfmt` ⚠️shared-Rust                                                               | `format-on-edit` `.rs` ⚠️shared; `stop-lint` `linux`; `block-generated` `target/` ⚠️shared | Linux row; Linux column; Linux STACK section                     |
...
| Rust CLI        | `rust-cli-development`                                 | `cargo`,`rustfmt` ⚠️shared-Rust                                                               | `format-on-edit` `.rs` ⚠️shared; `stop-lint` `tui`; `block-generated` `target/` ⚠️shared | High-Performance CLI STACK section; `apps/tui` line              |
