...
| [`scoped-commits.sh`](.claude/hooks/scoped-commits.sh)           | `PreToolUse` (Bash, gated to `git commit*`) | Requires a [Scoped Commits](https://scopedcommits.com/) subject (`<scope>: <description>`) whose scope is **defined** — a spec/feature `id:` (a reverse pointer), a platform, a harness area, or `treewide`. Rejects the Conventional `type(scope):` form. |
