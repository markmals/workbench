...
| [`scoped-commits.sh`](.claude/hooks/scoped-commits.sh)           | `PreToolUse` (Bash, gated to `git commit*`) | Requires a [Scoped Commits](https://scopedcommits.com/) subject (`<scope>: <description>`, e.g. `web:`, `vm.items.list:`, `specs:`) and rejects the Conventional `type(scope):` form.                                  |
