...
| Preview works                   | Load or install it, then exercise the changed behavior | A green workflow                               |
| Proposal implemented            | `mise run check` output: exit 0, this turn             | Code looks right                               |
| All proposal behavior addressed | Behavior inventory and proposal checklist              | Tests pass                                     |
...
```
✅ I ran `mise run <the-project-test-task>` — 47/47 tests pass. Implementation complete.
...
`mise run <the-project-test-task>` reports 2 failures:
